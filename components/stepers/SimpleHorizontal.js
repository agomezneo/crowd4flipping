  ...
  return (
    ...
    <div className={styles.steper_container}>
        ...
        {data.map((item, key) => (
            ...
            >
                <BsFillCheckCircleFill className={item.finalized ? styles.check_succes : styles.check} />
                <span className={ item.finalized ? `${styles.step_description} ${styles.description_success}` : `${styles.step_description}` }> {item.description} </span>  
            </motion.div>
        ))}  
